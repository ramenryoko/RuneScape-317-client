        ...
        try {
            ...
            for (int i = 0; i < frameCount; i++) {
                ...
                for (int base = 0; base < baseCount; base++) {
                    ...
                    // --- THE TRUE SAFE READ ---
                    // We MUST read from tran2 to keep the buffer perfectly aligned,
                    // even if Jagex's bone count is out of bounds.
                    int type = 0;
                    if (base < skeleton.baseTypes.length) {
                        type = skeleton.baseTypes[base];
                    }

                    if (type != SeqSkeleton.OP_BASE) {
                ...
                instances[frameID] = transform;
            }
        } catch (Exception e) {
            // Silently catch the 1 or 2 genuinely corrupted Jagex files.
            // Because the buffer stayed aligned above, the other 99% of files will load perfectly.
        }
    }
